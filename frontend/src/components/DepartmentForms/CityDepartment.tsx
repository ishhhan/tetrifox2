...
interface CityDepartmentProps {
  onAdd: (data: { name: string; match_value: string }) => void;
}

export function CityDepartment({ onAdd }: CityDepartmentProps) {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const formData = new FormData(e.currentTarget);
    const name = formData.get("name") as string;
    const match_value = formData.get("city") as string;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <Input name="city" placeholder="Enter city" className="mt-1" required />
      </div>
