...
interface PostalDepartmentProps {
  onAdd: (data: { name: string; match_value: string }) => void;
}

export function PostalDepartment({ onAdd }: PostalDepartmentProps) {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const formData = new FormData(e.currentTarget);
    const name = formData.get("name") as string;
    const match_value = formData.get("postal") as string;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <Input name="postal" placeholder="Enter postcode" className="mt-1" required />
      </div>
