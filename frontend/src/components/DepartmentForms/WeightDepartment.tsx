...
interface WeightDepartmentProps {
  onAdd: (data: { name: string; min?: number; max?: number }) => void;
}

// Max can be at most 100x min, with absolute max of 10000
// Max absolute limit
const MAX_LIMIT = 1000;
const RANGE_SPAN_LIMIT = 10;

export function WeightDepartment({ onAdd }: WeightDepartmentProps) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (finalMin !== undefined && finalMax !== undefined) {
      if (finalMax - finalMin > RANGE_SPAN_LIMIT) {
        setError(`Range cannot exceed ${RANGE_SPAN_LIMIT} kg`);
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <Label className="text-sm">Min (kg)</Label>
          <Input
            name="min"
            type="number"
            step="0.01"
        ...
        <div>
          <Label className="text-sm">Max (kg)</Label>
          <Input
            name="max"
            type="number"
            step="0.01"
