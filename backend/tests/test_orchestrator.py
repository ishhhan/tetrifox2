"""
Unit Tests for LogisticsOrchestrator.

Run with: pytest backend/tests/test_orchestrator.py
"""
import pytest
...
def test_process_success(orchestrator, mock_parser, mock_history, valid_request, mock_parcel):
    """Test standard success flow where parcel is routed and saved."""
    # Setup
...
def test_process_history_failure_trapped(orchestrator, mock_parser, mock_history, valid_request, mock_parcel):
    """Test that history save failure is logged but does NOT crash the request."""
    # Setup
    mock_parser.parse_with_stats.return_value = ([mock_parcel], ParsingStats())
    ...
    # Verify Success Response despite history failure
    assert response["status"] == "success"
    assert len(response["data"]) == 1
...
def test_validation_department_overlap(orchestrator, valid_request):
    """Test that invalid department rules raise AggregatedValidationError."""
...
def test_xml_parser_failure(orchestrator, mock_parser, valid_request):
    """Test that XML parsing failure raises AggregatedValidationError."""
    # Setup
...
def test_routing_engine_failure_caught(orchestrator, mock_parser, valid_request, mock_parcel):
    """Test that RoutingEngine init failures (like bad config) are caught."""
    # Setup
    mock_parser.parse_with_stats.return_value = ([mock_parcel], ParsingStats())
    
    # Mock RoutingEngineService to raise ValueError on init
    # Note: Since RoutingEngineService is instantiated inside process, we patch it
